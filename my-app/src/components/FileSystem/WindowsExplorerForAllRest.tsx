...
const WindowsExplorerForAllRest = ({ onNavigate, root, filename }) => {
    const [isSearchFocused, setIsSearchFocused] = useState(false);
    const [currentTime, setCurrentTime] = useState(new Date());
    const [isScrolled, setIsScrolled] = useState(false);
    const [drives, setDrives] = useState([]);
    const [rootTemp, setRoot] = useState([]);

    useEffect(() => {
        fetch(`http://127.0.0.1:5000/partition/${root}/${filename}`)
            .then((response) => response.json())
            .then((data) => {
                console.log("Fetched drives:", data.files);
                setDrives(data.files);  // Ensure 'folders' is correct
                setRoot(data.root);
                root = rootTemp
                console.log("Root ", rootTemp);
            })
    ...
    }, [rootTemp]);  // Runs whenever `root` changes
    ...
    return (
        <div className="h-screen bg-[#0A0A0F] bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white overflow-hidden">
            ...
            <MainContent3 sidebarItems={sidebarItems} drives={drives} handleScroll={handleScroll} onNavigate={onNavigate} />

        </div>
    );
};

export default WindowsExplorerForAllRest;