...
const WindowsExplorer = ({ onNavigate }) => {
    ...
    useEffect(() => {
        fetch("http://127.0.0.1:5000/partitions")
            .then((response) => response.json())
            .then((data) => {
                console.log("Fetched drives:", data);
                setDrives(data.folders);  // Ensure 'folders' is correct
            })
    ...
    return (
        <div className="h-screen bg-[#0A0A0F] bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white overflow-hidden">
            ...
            <MainContent sidebarItems={sidebarItems} drives={drives} handleScroll={handleScroll} onNavigate={onNavigate} />
